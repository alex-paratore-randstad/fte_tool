...
type WeeklyForecastTableProps = {
  currentDate: Date | null;
  refreshKey: number;
};

export function WeeklyForecastTable({ currentDate, refreshKey }: WeeklyForecastTableProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const allocationRequests = weekKeys.map(async weekKey => {
        const response = await fetch(`/domo/datastores/v1/collections/weekly_forecast/documents?q=content.forecast_allocation_date='${weekKey}'`);
        if (!response.ok) {
            console.warn(`Failed to fetch forecasts for ${weekKey}. This may be expected in local dev.`);
            return [];
        };
      ...
      const allFetchedAllocations: WeeklyForecast[] = results.flat();
      
      const groupedByEmployee = allFetchedAllocations.reduce((acc, current) => {
        const { forecast_allocation_name, forecast_cost_center_number, forecast_cost_center_name, forecast_allocation_date, forecast_allocation_amount } = current.content;
        
        if (!acc[forecast_allocation_name]) {
            acc[forecast_allocation_name] = {};
        }
        if (!acc[forecast_allocation_name][forecast_cost_center_number]) {
            acc[forecast_allocation_name][forecast_cost_center_number] = {
                clientId: forecast_cost_center_number,
                clientName: forecast_cost_center_name,
                weeklyFtes: {},
            };
        }
        acc[forecast_allocation_name][forecast_cost_center_number].weeklyFtes[forecast_allocation_date] = {
          fte: parseFloat(forecast_allocation_amount) || 0,
          docId: current.id,
        };
        return acc;
  ...
  const handleSaveChanges = async () => {
            ...
            if (original && editable.fte !== original.fte) {
              updates.push({
                ...
                content: {
                  ...
                  forecast_allocation_name: empAlloc.employeeName,
                  forecast_cost_center_name: alloc.clientName,
                  forecast_cost_center_number: alloc.clientId,
                  forecast_allocation_amount: (editable.fte || 0).toString(),
                },
              });
            }
          }
    ...
    try {
      await Promise.all(updates.map(update =>
        fetch(`/domo/datastores/v1/collections/weekly_forecast/documents/${update.docId}`, {
          method: 'PUT',
      ...
      toast({ title: 'Success', description: `${updates.length} forecast(s) updated.` });
  ...
  if (loading || !startOfCurrentWeek || !currentDate) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Saved Forecasts for this Period</CardTitle>
          <CardDescription>Records from the weekly_forecast collection for the displayed weeks.</CardDescription>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <CardTitle>Saved Forecasts for this Period</CardTitle>
              <CardDescription>Records from the weekly_forecast collection. You can edit values and save.</CardDescription>
            </div>
                ...
                {filteredAllocations.length === 0 ? (
                    <TableRow>
                        <TableCell colSpan={weeks.length + 1} className="text-center h-24 text-muted-foreground">
                            {nameFilter ? 'No matching employees found.' : 'No saved forecast data found for this period.'}
                        </TableCell>
                    </TableRow>
                ) : (
