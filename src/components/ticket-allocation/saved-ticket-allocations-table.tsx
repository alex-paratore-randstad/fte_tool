...
type SavedTicketAllocationsTableProps = {
  refreshKey: number;
};

export function SavedTicketAllocationsTable({ refreshKey }: SavedTicketAllocationsTableProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const grouped = summaries.reduce((acc, summary) => {
        const { ticket_allocation_id, cost_center_name, cost_center_number, allocation_percentage, ticket_allocation_date } = summary.content;
        
        if (!acc[ticket_allocation_id]) {
          acc[ticket_allocation_id] = {
            id: ticket_allocation_id,
            ticket_allocation_date: ticket_allocation_date,
            employees: [],
            summaries: [],
          };
        }
      ...
      ftes.forEach(fte => {
        const { ticket_allocation_id, employee_name, allocation_monthyear } = fte.content;
      ...
      const processed = Object.values(grouped).sort((a, b) => new Date(b.ticket_allocation_date).getTime() - new Date(a.ticket_allocation_date).getTime());
    ...
    const updates = editableAlloc.summaries
      ...
      .map(summary => ({
          ...
          content: {
              ...
              ticket_allocation_date: editableAlloc.ticket_allocation_date,
          }
      }));
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Saved Ticket Allocation Profiles</CardTitle>
        <CardDescription>History of all saved ticket allocation profiles. You can edit percentages here.</CardDescription>
      </CardHeader>
      <CardContent>
        {editableAllocations.length === 0 ? (
          <div className="text-center text-muted-foreground py-10">
            No ticket allocation profiles found.
          </div>
        ) : (
              ...
              <AccordionItem value={alloc.id} key={alloc.id}>
                <AccordionTrigger>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                        ...
                        <Badge variant="secondary">{alloc.employees.length} Agents</Badge>
                        <span className="text-sm text-muted-foreground hidden sm:inline">
                            Created: {new Date(alloc.ticket_allocation_date).toLocaleDateString('en-US', { timeZone: 'UTC' })}
                        </span>
                    </div>
                </AccordionTrigger>
                <AccordionContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-muted/50 rounded-md">
                        ...
                        <div>
                             <h4 className="font-semibold mb-2">Agent Group Allocation</h4>
                             <Table>
                                <TableHeader>
                                    <TableRow>
