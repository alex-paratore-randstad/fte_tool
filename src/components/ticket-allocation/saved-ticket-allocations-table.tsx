...
export function SavedTicketAllocationsTable({ refreshKey }: SavedTicketAllocationsTableProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const grouped = summaries.reduce((acc, summary) => {
        ...
        if (!acc[ticket_allocation_id]) {
          acc[ticket_allocation_id] = {
            ...
            allocationDate: '', // Will be populated from FTE records
            employees: [],
            summaries: [],
          };
        }
      ...
      ftes.forEach(fte => {
        const { ticket_allocation_id, employee_name, ticket_allocation_date, allocation_monthyear } = fte.content;
      ...
      const processed = Object.values(grouped)
        .filter(g => g.allocationDate) // Ensure record has a date before sorting
