            ...
            <div className="flex h-full flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-4">
                <nav className="grid gap-2 text-lg font-medium">
                  <Link href="/" className={cn("flex items-center gap-4 rounded-xl px-3 py-2 text-base", isActive('/') ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground")}>
                      Dashboard
                  </Link>
                                  ...
                                  <AccordionContent className="pl-4 pb-0">
                                      <div className="flex flex-col gap-1">
                                          {(group.items || []).filter(item => item && userHasAccess(item.roles)).map(item => (
                                              <Link key={item.href} href={item.href} className={cn("block rounded-lg p-3 text-sm", isActive(item.href) ? "bg-muted font-semibold text-foreground" : "text-muted-foreground hover:bg-muted/50 hover:text-foreground")}>
                                                  {item.label}
                                              </Link>
                                          ))}
                                      </div>
