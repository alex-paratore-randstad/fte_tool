...
export function TicketAllocationGrid({ onSaveSuccess }: TicketAllocationGridProps) {
  ...
  const fetchDataAndPrepopulate = useCallback(async () => {
    ...
    try {
      ...
      const allTicketData: TicketAllocationData[] = await response.json();

      const filteredData = allTicketData.filter(
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        toast({
            ...
            description: `${submissions.length} allocation entries for ${currentDate ? format(currentDate, 'MMM yyyy') : ''} have been saved successfully.`,
        });
        onSaveSuccess();
    } catch (error: any) {
